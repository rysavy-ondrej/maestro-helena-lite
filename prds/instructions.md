...
> **This is a copy.** The canonical build rules live in
> [`../concept/instruction.md`](../concept/instruction.md), and its relative links
> resolve there but not here. Read the original; this copy exists only so the
> folder is self-contained, and **a second copy of a record is a record that can
> drift.** If the two differ, the original wins and the difference is a defect.
