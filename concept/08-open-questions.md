...
| **Measured 2026-09-05, no longer unmeasured**: over `data/connections/network-day/20250920` (143 captures, 239 850 records, 3 199 hosts, 24 h) a context holds a median of **1** entity row, p90 3, p99 441, and a maximum of **1 822** (1 051 address, 751 domain). Most host-windows are nearly empty and a small tail is enormous, so a bound taken from the mean of 14.6 is wrong by two orders of magnitude on the busiest contexts — which are the ones triage is for. A six-hour slice of the same capture peaked at 795, so the tail is not visible in part of a day. `demo/context_over_a_day.py` prints the distribution, so the number is re-derivable rather than quoted | A bound is chosen against this distribution |
...
| **The pre-triage gate clears what nothing read** | Added 2026-09-13 with the gate itself. A context below the configured indicator count is emitted as `normal` without a model having read it, so the verdict asserts a safety no run established — and because feed coverage is a sparse sighting window, this is the ordinary path and not the edge case |
